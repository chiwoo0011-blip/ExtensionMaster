...
function uid() { return 'id_' + Date.now() + '_' + Math.random().toString(36).substring(2, 8); }
...
var App = {
    ...
    bindEvents: function() {
        ...
        document.getElementById('adminClearAll').addEventListener('click', function() {
            if (!confirm('정말로 모든 연락처를 삭제하시겠습니까?')) return;
    ...
    saveContactFromModal: async function() {
        ...
        var contact  = {
            ...
            order:   existing ? (existing.order || 0) : Object.keys(this.state.contacts).length
        };
    ...
    saveRoomFromModal: async function() {
        ...
        var room = {
            ...
            order:     existing ? (existing.order || 0) : Object.keys(this.state.rooms).length
        };

        this.state.rooms[room.id] = room;
        await ContactDB.saveRoom(room);
        document.getElementById('roomModal').classList.add('hidden');
        Renderer.render();
    ...
    scrollToContact: function(id) {
        ...
        if (App.state.filterFloor && App.state.filterFloor !== contact.floorId) {
